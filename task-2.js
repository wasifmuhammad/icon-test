...
const server = http.createServer((req, res) => {
  ...
  if (
    parsedUrl.pathname === "/I/want/title/" &&
    parsedUrl.searchParams.has("address")
  ) {
    const addresses = parsedUrl.searchParams.getAll("address");
        ...
        http
          .get(`http://${address}`, (response) => {
            ...
            response.on("end", () => {
              ...
              resolve({
                address: address,
                title: title ? title[1] : "response not getter",
              });
            });
          })
          .on("error", (e) => {
